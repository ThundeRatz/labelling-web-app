...
def get_one_pending():
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(
            "SELECT id FROM pending OFFSET floor(random() * (SELECT COUNT(*) FROM pending)) LIMIT 1"
        )
...
def set_label(image, labels):
    ...
    with conn.cursor() as cursor:
        ...
        if labels_valid:
            db_labels = (
                (image, x["x"], x["y"], x["width"], x["height"]) for x in labels
            )
            cursor.executemany(
                "INSERT INTO new_labels VALUES (%s, %s, %s, %s, %s)", db_labels
            )
    if labels_valid:
        conn.commit()
    conn.close()
